...
\ks{are these four using deep features, or are we just copying their results?}
...
\ks{in the intro we said Imagenet is better than Office, but that dataset bias
nonetheless remains a major issue. can we state that result here pointing to table3? Also, strictly speaking Imagenet is not better than Office: for 3 examples, Imagenet to Webcam gets 81 judging from fig2,  we should discuss why that does not beat the highest number in table1, 86 for DSLR to Webcam. The lesson here is that if the source domain is very close to target, then it is better than a large scale but more distinct source}

\ks{we also said that ``we achieve target domain performance on par with source domain performance using only a single labeled example per target category'', where is the source domain performance number here?}
...
\ks{Why not repeat the numbers from Background section here?}
